  ...
  const handleSubscribe = async (plan = 'pro') => {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          plan,
          successUrl: `${frontendBase}${appPath}?payment=success`,
          cancelUrl: `${frontendBase}${appPath}?payment=cancelled`,
        }),
      })
      ...
      <header className="border-b border-slate-200 bg-white/80 backdrop-blur-sm shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Link to="/" className="text-sm text-slate-500 hover:text-slate-700 font-medium">Home</Link>
            <button
              type="button"
